...
class Loss(nn.modules.loss._Loss):
    def __init__(self, args, ckpt):
        ...
        for loss in args.loss.split('+'):
            ...
            elif loss_type == 'KDLoss_vit':
                loss_function = KDLoss()
            elif loss_type == 'RKDLoss':
    ...
    def forward(self, outputs, labels=None, epoch=None, teacher_outputs=None, teacher_outputs2=None):
        losses = []
        loss_part=[]
        g_prec=0
        for i, l in enumerate(self.loss):
            if self.args.model == 'MGN' and l['type'] == 'Triplet':
                ...
                l_w = [1 for x in range(len(outputs['fc_logits']))]
                loss = [l['function'](output, labels) * l_w[i] for i, output in enumerate(outputs['fc_logits'])]
                loss_part.extend(loss)
                loss = sum(loss) / len(l_w)
                effective_loss = l['weight'] * loss
                losses.append(effective_loss)
                self.log[-1, i] += effective_loss.item()
            else:
        ...
        return loss_sum, torch.tensor(loss_part), g_prec
