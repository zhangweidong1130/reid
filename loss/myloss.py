...
def loss_fn_kd(outputs, teacher_outputs_1, teacher_outputs_2=None):
    alpha = 0.6
    T = 1
    kd_loss = nn.KLDivLoss(reduction='batchmean')
    teacher_outputs_softmax = 1*F.softmax(teacher_outputs_1/T, dim=1)
    KD_loss = kd_loss(F.log_softmax(outputs/T, dim=1), teacher_outputs_softmax)*T*T
...
class KDLoss(nn.Module):
    ...
    def forward(self, outputs, teacher_outputs_1, teacher_outputs_2=None, **kwargs):
        return loss_fn_kd(outputs, teacher_outputs_1)
